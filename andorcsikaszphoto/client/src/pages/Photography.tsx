...
function GalleryItem({
  ...
  return (
    <motion.div
      ...
      whileHover={reduced ? undefined : { y: -3, transition: { duration: 0.22, ease: "easeOut" } }}
    >
      <button
        type="button"
        onClick={onClick}
        className="group relative w-full overflow-hidden rounded-xl sm:rounded-2xl bg-muted/50 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background shadow-md hover:shadow-xl hover:shadow-black/25 transition-shadow duration-300"
        style={{ aspectRatio }}
      >
        <img
          src={item.src}
    ...
    <div className="min-h-screen pb-24 sm:pb-32">
      ...
      <section className="container pt-16 sm:pt-20 pb-16 sm:pb-24 scroll-mt-20">
        <div className="grid gap-12 lg:grid-cols-5 lg:gap-16 items-start">
          ...
          >
            <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground/60 font-medium mb-5">
              Photography
            </p>
            <h1 className="font-headline text-3xl sm:text-4xl md:text-5xl lg:text-6xl tracking-tighter text-foreground leading-[0.92]">
      ...
      <section className="container py-16 sm:py-20 border-t border-border/40">
        <div className="flex items-baseline gap-4 mb-10">
          <motion.h2
            className="font-headline text-3xl sm:text-4xl lg:text-5xl uppercase tracking-tighter text-foreground"
          ...
          <motion.span
            className="text-sm text-muted-foreground/60 font-mono tracking-wider tabular-nums"
            initial={reduced ? false : { opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ delay: 0.1 }}
          >
            {photographyItems.length} photos
          </motion.span>
        </div>
      ...
      <section className="container py-16 sm:py-24 border-t border-border/40">
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          {photographyServices.map((svc, i) => (
            <motion.div
              key={svc.id}
              className="group rounded-xl sm:rounded-2xl border border-border/60 bg-card/80 backdrop-blur-sm p-6 sm:p-8 hover:border-border hover:bg-card transition-all duration-300"
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <span className="h-px flex-1 bg-border/50 group-hover:bg-primary/40 transition-colors duration-300" />
                <span className="text-xs font-mono text-muted-foreground/50 tabular-nums">0{i + 1}</span>
              </div>
      ...
      <section className="container py-16 sm:py-24 border-t border-border/40">
        <div className="grid gap-10 lg:grid-cols-5 items-start">
          ...
          <motion.ul
            ...
            viewport={{ once: true }}
          >
            {["Wedding, event, and travel sessions", "Fast reply, clear pricing, simple process", "Flexible date and location planning"].map((item, i) => (
